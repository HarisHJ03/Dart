...
static void CANAddFilter(CANInstance *_instance)
{
    ...
    else    //由于小米的垃圾协议，拓展帧里面也藏了数据，每次接收的拓展帧都不一样，所以拓展帧需要用mask模式，也就是匹配模式                                                                              // 如果为拓展帧
    {
        /* 
            （一）：
                发现问题：这里这样是不利于程序封装的，但是目前也只有小米电机用到了EXTID，并且把真实ID藏到了EXTID中，所以这里统一使用小米的ID格式
                解决方案：如果后续有电机也适用EXTID，可以用switch判断一下CAN实例的拥有者是什么类型，使用不同的匹配规则
